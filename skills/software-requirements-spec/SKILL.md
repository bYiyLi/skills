...
对单一软件需求文档执行用户请求的操作，并将文档结果、只读审校结果或真实阻塞交回当前 host task。本 Skill 不承担设计、实现、测试、提交或发布；宿主继续本次请求已经明确授权的后续步骤，不把本 Skill 结束当作整个任务结束。
...
本文的“确认”和“分别授权”指核对每项动作已有的明确依据，不要求用户逐项重复批准；同一本次请求可以授权多个动作。即时确认、正式状态变更和删除的明确要求仍须遵守。

如果请求在 `review` 与 `revise` 之间不明确，先保持只读并检查可用材料；只有歧义仍阻止请求结果时才澄清，明确授权前不写入。若请求仍需在 requirements、design、usage 或 reference 等文档类型间判型，先使用 `software-doc-writing-standards`；仅在结果是单一 requirements artifact 时继续。多类文档需要联动时由 `sync-software-docs` 拥有组合结果；当它要修改 requirements 正文时与本 Skill 有意共用，不要在本 Skill 内接管整套文档同步。
...
- **Post-start failure**：先检查当前文件和仓库状态。仅对确认尚未执行、可安全重试且仍在授权范围内的动作，修正原因后继续；不盲目重复已成功或结果未知且可能重复产生副作用的动作。不能安全恢复时只阻塞依赖它的部分，继续其他独立的已授权工作，报告已完成状态、失败动作、未执行验证和恢复条件。权限拒绝仍按 Denied 处理，不属于可重试故障。
...
不要把建议的下一步当作已完成动作。commit、push、deploy 或外部发送必须有覆盖该动作的明确授权，并由当前 host task 执行和验证；同一本次请求已明确授予该授权时，不因本 Skill 结束而再次索取授权。明确的动作前即时确认要求仍须遵守。
