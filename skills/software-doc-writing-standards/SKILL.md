...
- 需要确定文档该保存到哪里、如何命名、如何标状态和归档时，读 [references/doc-management-contract.md](references/doc-management-contract.md)。
...
3. 所有软件文档都必须有 canonical repository placement。
   - 先判断应落在哪个文档目录，再开始写正文。
   - 有现成真源文档就优先更新，不要平行新建重复文件。
   - 没有显式仓库合同就使用本仓库的默认 `docs/` 布局。
4. 所有软件文档都必须明确以下最小上下文。
   - audience：写给谁。
   - scope：覆盖什么，不覆盖什么。
...
5. 所有软件文档都必须有最小管理元信息。
   - 至少明确 doc type、status、owner、last updated、baseline。
   - 被替代文档要写清 `supersedes / superseded_by`。
6. 需求类文档必须满足“可判断、可验证、可追溯”。
   - 写成单一、必要、清晰、可测试的要求。
   - 非设计约束不要伪装成需求。
   - 每条关键要求都要能追到来源和预期验证方式。
7. 设计类文档必须满足“有驱动、有视图、有取舍”。
   - 先写驱动因素、质量目标、约束和边界。
   - 再写结构、接口、数据、运行时行为、部署和故障处理。
   - 每个重要设计都要解释 rationale 和 trade-off，而不是只列结果。
8. 使用与参考类文档必须满足“面向任务或面向查询，而不是面向作者叙述习惯”。
   - task docs 用步骤、前置条件、预期结果、回退和排障。
   - reference docs 直接镜像真实接口面，列语法、参数、返回、错误、示例、适用版本。
   - explanation 只讲 why，不伪装成操作步骤。
9. 缺证据时默认保守，而不是补写想当然的内容。
   - 缺真源就标假设。
   - 缺结论就列 open question。
...
3. 目标仓库已有显式文档合同，优先于本 skill 的默认目录建议。
   - 但不能破坏真源唯一、状态明确、关系可追溯这三条硬约束。
4. 用户给定模板优先于本 skill 的建议结构，但不能破坏可验证、可追溯、不可编造这三条硬约束。
5. 规范正文优先于示例。
   - 示例只能帮助理解，不能推翻正文规则。
6. 无法裁决时，明确标“不确定”并停止声称这是最终版。
...
3. 明确说明文档的 canonical path、状态和是新建还是更新。
4. 标出关键事实来自哪类真源。
5. 单独列出 assumptions、gaps、open questions，不把它们伪装成定论。
6. 如果文档替代旧文档，明确说明 supersede 关系和旧文档处理方式。
7. 评审输出时，结论能对应到具体规则或规则组，而不是只给“建议更清晰”。
...
5. 目标仓库存在互相冲突的文档合同，且当前无法判断哪套管理规则为准。
