...
到达请求终点后结束本 Skill 的职责并返回当前 host task，继续本次已授权的剩余步骤；
本 Skill 不从文档请求推导实现代码、提交、推送、发布或部署授权。
...
尚未加载是要执行的准备步骤，不是 blocker。只有对应 Skill 或必需资源确实不可用、
加载失败，或与更高优先级约束冲突时，才阻塞依赖它的正文动作，继续其他独立的已授权
动作。提到名称不证明已经加载或应用。impact、plan 和只需 lifecycle 的动作不依赖
正文 Skill；不得从通用知识重建缺失的规范，也不得绕过宿主的加载权限。
...
以及这些判断适用的版本、分支、发布状态或时间点。先从请求和已授权来源取得会改变
影响范围的事实；仍缺少决定性信息时，只阻塞依赖它的判断并请求最小补充。继续完成
独立判断，把未决维度记录为 `unresolved`，不把未知类别标为 `no-change`。
...
- `sync`：只执行矩阵中已授权且依赖已满足的动作。创建或更新正文时，按前述规则主动
  加载并应用对应文档 Skill；确实无法取得必需规范时，只将依赖它的正文动作标为
  blocker。继续其他独立且已授权的动作，保留已经完成的结果。

任何计划文件或目标文档的写入、metadata/状态变更、移动、归档或删除中途失败时，重新检查
每个已开始动作的准确目标和当前仓库状态。仅对确认尚未执行、可安全重试且仍在授权
范围内的动作，修正原因后继续；不盲目重复已成功或结果未知且可能重复产生副作用的
动作。无法安全恢复时只阻塞依赖它的部分，继续其他独立的已授权动作。权限拒绝不属于
可重试故障。报告已完成项、失败项、未执行项、未完成验证和恢复条件。
...
阻塞条件。到达所选终点后结束本 Skill 的职责，把结果交回当前 host task；本次已授权
的后续任务继续执行，未授权的提交、推送、发布或实现不执行。
